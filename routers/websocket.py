...
async def handle_reply_message(data: dict, room_id: str, current_user, db):
    ...
    await manager.broadcast(
        room_id,
        {
            ...
            "user": {"display_name": current_user.display_name, "username": current_user.username},
            "reply": {
                "id": replied_user.id,
...
async def handle_new_message(data: dict, room_id: str, current_user, db):
    ...
    await manager.broadcast(
        room_id,
        {
            ...
            "room_id": room_id,
            "created_at": str(new_message.created_at),
            "user": {"id": current_user.id, "display_name": current_user.display_name, "username": current_user.username},
        },
    )
